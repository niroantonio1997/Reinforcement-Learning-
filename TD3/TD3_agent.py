...
class TD3Agent: 
    ...
    def __init__(self, hyperparameters_set):
        ...
        with open('TD3/hyperparameters.yml','r') as file:
        ...
        self.noise_clip = 0.5    # Target noise clipping
        self.update_counter = 0  # Counter for delayed updates
    ...
    def run(self, is_training):
        ...
        if is_training:
            ...
            signal.signal(signal.SIGINT, create_signal_handler(self, "TD3"))
            ...
            self.critic_1 = net(obs_space + action_dim, self.critic_hidden_dims, 1).to(device)
            self.critic_2 = net(obs_space + action_dim, self.critic_hidden_dims, 1).to(device)
            self.actor_target = net(obs_space, self.actor_hidden_dims, action_dim, output_activiation=torch.tanh).to(device)
            self.critic_1_target = net(obs_space + action_dim, self.critic_hidden_dims, 1).to(device)
            self.critic_2_target = net(obs_space + action_dim, self.critic_hidden_dims, 1).to(device)
            ...
            for episode in range(self.max_episodes):
                ...
                while not done and steps < self.max_episode_steps:
                    ...
                    # Store experience
                    self.replay_buffer.store(obs, action, reward, next_obs, done, 0.0)  # log_prob=0 per TD3
                    ...
                    if self.replay_buffer.size >= self.batch_size and total_steps % self.train_freq == 0:
                        ...
                        self.optimize(mini_batch, total_steps)
                ...
                if save_video and len(frames) > 0:
                    save_episode_video(frames, episode, self.hyperparameters_set, self.env_id, "TD3")
            ...
            # Save best models
            torch.save(self.best_actor.state_dict(), f'TD3/nets/{self.env_id}_actor.pth')

            # Plot results
            plot_results(self.episodes_reward, self.env_id, self.max_average_reward, "TD3")
            ...
            # Load trained model
            self.actor = net(obs_space, self.actor_hidden_dims, action_dim, output_activiation=torch.tanh).to(device)
            self.actor.load_state_dict(torch.load(f'TD3/nets/{self.env_id}_actor.pth'))
            self.actor.eval()
    ...
    def optimize(self, mini_batch, step):
        """
        Optimize actor and critic networks using TD3
        """
        ...
        # CRITIC UPDATE (sempre)
        with torch.no_grad():
            # Target Policy Smoothing - caratteristica chiave di TD3
            noise = torch.clamp(torch.randn_like(actions) * self.target_noise, -self.noise_clip, self.noise_clip)
            next_actions = torch.clamp(self.actor_target(new_states) + noise, -1, 1)
            
            # Clipped Double Q-Learning
            q_target_1 = self.critic_1_target(torch.cat((new_states, next_actions), dim=1))
            q_target_2 = self.critic_2_target(torch.cat((new_states, next_actions), dim=1))
            q_target = torch.min(q_target_1, q_target_2)  # Take minimum (pessimistic)
            q_target = rewards + (1 - terminations) * self.gamma * q_target

        # Update both critics
        q_1 = self.critic_1(torch.cat((states, actions), dim=1))
        q_2 = self.critic_2(torch.cat((states, actions), dim=1))
        critic_loss_1 = nn.MSELoss()(q_1, q_target)
        critic_loss_2 = nn.MSELoss()(q_2, q_target)
        ...
        # ACTOR UPDATE (delayed) - caratteristica chiave di TD3
        if step % self.policy_delay == 0:
            # Use only critic_1 for actor loss
...
def main():
    ...
    hyperparameters_set = "ant"  # Change this to desired environment
    
    os.makedirs('TD3/video/' + hyperparameters_set, exist_ok=True)
    os.makedirs('TD3/graphs', exist_ok=True)
    agent = TD3Agent(hyperparameters_set)
